...
- **Streams can be scheduled to go live at a set time**, from the new **File > Schedule stream...**. Pick **Simple** to give one date and time — Pubsplash connects then and broadcasts whichever scene is active. Pick **Advanced** to give two times and two scenes: Pubsplash connects at the pre-stream time on the first scene, then switches to the second at the start of stream time, when your content actually begins. That is worth doing because Audio Pub checks a new source before it serves anyone, so connecting a few minutes early on a music scene means listeners arrive to a stream that is already working. A start of stream time earlier in the day than the pre-stream time is taken as the following morning, so a broadcast can cross midnight.

- While a stream is scheduled, the Home tab's first line reads "Stream scheduled" and a second line counts down in words to the connection, then to the scene switch. The **Start streaming** button becomes **Cancel scheduled stream**, and recording is unavailable — from the button and from a keybinding — so nothing can be in the way when the stream starts. Once it connects, Home behaves exactly as it does for a stream you started yourself. Stopping the stream discards anything the schedule had left, and a scheduled switch changes the active scene for good, the same as switching by hand does.

- A schedule lasts for the current session only. It is not saved, so Pubsplash can never start broadcasting at launch, and closing the window asks before discarding one. If the computer was asleep when the time came round, Pubsplash cancels the schedule rather than going live more than two minutes late, and says so in the log and out loud. Automatic update questions now stand down while a stream is scheduled, as they already did while one was running.

- **Pubsplash's own playback device can now be chosen**, on the new **Audio** tab in Preferences. Everything Pubsplash plays for you goes to it — sources you are monitoring, text-to-speech, and sound cues — so you can monitor on headphones while the rest of the machine keeps using the speakers. It is not what listeners hear, and changing it does not touch your stream. Leave it on "Default output device (follow system)" and Pubsplash behaves exactly as it did, moving with Windows when you plug in a headset. **Play a test sound** plays the startup sound out of the chosen device so you can check it without starting a stream. If the device you pick is not available when Pubsplash comes to use it, it says so in the log and keeps trying rather than quietly using a different one.

- **A Desktop Audio source can now capture one output device** instead of all of them. Desktop Audio has an edit dialog for the first time — pressing Edit on one used to say it had no settings — offering "All output devices (Pubsplash excluded)" and each of your playback devices by name. The default is unchanged, and an existing Desktop Audio source keeps it. The one device you cannot pick is the one Pubsplash itself plays out of: capturing a single device captures *everything* on it, so aiming it there would send Pubsplash's own speech and sound cues back out to your listeners. Pubsplash refuses that pairing and says why, leaving the dialog open so you can choose another. If you later change Pubsplash's output device to one a Desktop Audio source is capturing, Pubsplash tells you and that source goes back to capturing every device with Pubsplash excluded, so its own audio can never reach the stream by either route.
...
- **The date and time controls in Schedule stream now announce each part as you arrow across them**, naming it and reading its value — "hour, 8", "month, August" — so you can tell what pressing Up or Down is about to change. Windows says nothing at all as the highlight moves between the parts of one of these controls, which left no way to know where you were. Up, Down, Home and End re-read the part you are on with its new value. Pressing Home or End sets that part to its lowest or highest value rather than moving between parts, which is Windows' own behaviour and is now audible. Choosing a part with the mouse works too. Which parts a control has is read from the control itself rather than from your regional settings, so a 24-hour clock is announced as one and a 12-hour clock with AM and PM as the other, and the parts are named in the order they are actually shown.

- **Pubsplash no longer says "Streaming" before Audio Pub has accepted the stream.** Opening the connection to the streaming server is not the same thing as listeners being able to hear you: Audio Pub checks the audio you are sending before it serves anyone, and until that finishes, a stream page plays silence. Pubsplash reported a healthy broadcast throughout, which is why starting a stream and pressing play on its page could give half a minute of nothing with no sign anything was wrong. The Home tab's Status now reads **"Streaming (waiting for the server to accept the stream)"** until the server confirms it, then drops the note; if the server later loses the source while your own connection still looks fine, it reads **"Streaming (the server has lost the source)"**. The spoken announcement follows the same rule — "Connected, waiting for the server" when the stream connects, and "Streaming started" only once it is genuinely being heard. A stream that has waited three quarters of a minute puts an explanation in the log. If the connection carrying the server's answer goes down, Pubsplash stops reporting on the server's view instead of leaving the wait on screen: a broadcast your listeners can hear is never held at "waiting for the server" merely because the connection that would have confirmed it is missing. None of this changes what is sent, and a direct Icecast mount, which has no such check, is unaffected.

- **A stream no longer starts several seconds behind live.** Pubsplash begins encoding the moment you press Start, before the connection to the streaming server has been made, so the audio recorded during those opening moments was already stale by the time it could be sent — and sending it first is what set every listener's starting point. That backlog is now discarded when the connection opens, exactly as it already was after a reconnect, so a broadcast begins at the current moment rather than at the moment you pressed the button.

- The log now records how long each stage of starting a stream took — creating the stream, the streaming server accepting the connection, and the server accepting the audio — so a slow start can be told apart from a failing one.

- A Desktop Audio source pinned to a single output device no longer crackles. It read one buffer of audio from the device per turn while checking for more only ten times a second, and Windows will not wake a program on that schedule any more precisely than roughly every sixteen milliseconds — so the source fell a little further behind on every turn, with no way to catch up, and Windows discarded the audio that would not fit. It now takes everything waiting for it each time it looks, and asks Windows for a larger cushion to hold it, so a late moment costs a fraction of a second of delay instead of a hole in the sound.
