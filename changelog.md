...
- Per-source sends: each source has a "Sends..." dialog (Scenes and Sources tab) choosing which buses it feeds and at what level, plus a "Send directly to master" checkbox â€” on (the previous behavior) for aux-style dry+FX mixes, off to route a source only through its buses.
...
- FX chain library: save the selected chain under a name, load a saved chain, and delete saved chains â€” all stored together in `%LOCALAPPDATA%\pubsplash\fx_chains.json`. Chains can be exported to a standalone `.pubfx` file and imported on another machine to share setups.
...
- The Set stream info dialog now has a Quality dropdown to choose the MP3 encoder bitrate (48â€“320 kbps). Unlike the title and description, this setting is saved in the configuration file and persists across sessions.
