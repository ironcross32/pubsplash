...
**Preview sounds** on the same tab opens a window that lists every sound event Pubsplash has — the startup and shut-down cues, and the five a Sound Events source can react to — and plays the chosen pack's sound for whichever one you select. Arrow to an event and press `SPACE` to hear it, or use the **Play** button beside the list. `SPACE` on the event that is already playing stops it, and `SPACE` after arrowing to a different one cuts the first sound off and starts the new one at once, so comparing two events never means waiting one out. The button says **Stop** only while pressing it would stop something — that is, while the sound playing is the one you are sitting on; it goes back to **Play** on its own when the sound ends, and as soon as you arrow to another event. A pack does not have to have a sound for every event, and if the one you picked has none, Pubsplash says so rather than going quiet. Previews only ever play on your own default output device; they never reach the stream or a recording. This is the one button on the tab without an `ALT` shortcut, because every letter of the word is already claimed by another tab of the same dialog.
